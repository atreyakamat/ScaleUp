...
**ScaleUp** is a high-performance, self-hosted, privacy-first local image enhancement and batch super-resolution application. Built with a sleek, dark-blue glassmorphic UI for Linux desktop environments (Arch Linux + Hyprland), it harnesses **NCNN-Vulkan** C++ compute to execute hardware-accelerated deep learning models directly on integrated and discrete AMD GPUs via Mesa's RADV driver.
...
- **🌌 Modern Dark Blue UI:** Tailored sapphire-and-navy aesthetic with interactive glassmorphism, responsive controls, and smooth animations.
...
- **🤖 4 Pre-Trained Deep Learning Models:**
  1. `realesrgan-x4plus` — General photorealistic super-resolution (2x, 4x).
  2. `realesrnet-x4plus` — Denoise & smooth gradient enhancement (2x, 4x).
  3. `realesrgan-x4plus-anime` — Anime, manga, UI vectors, and flat illustration upscaling (4x).
  4. `realesr-animevideov3` — Ultra-low latency model for high-throughput batch workloads (2x, 3x, 4x).
- **🛡️ Memory Safeguards & Dynamic OOM Recovery:** Pre-flight dimension inspection warns and auto-clamps tile size for >4K images. Automatic tile-halving fallback ($128\text{px} \to 64\text{px} \to 32\text{px}$) retries images automatically if GPU memory limit is approached or black shader outputs are detected.
...
### Run with Docker Compose (GPU Accelerated)

```bash
docker compose up -d
```

### Or Build & Run with Docker CLI

```bash
# Build Docker image
docker build -t scaleup:latest .
